...
if auto_refresh:
    refresh_count = st_autorefresh(interval=AUTO_REFRESH_MS, key="companies_house_refresh")
else:
    refresh_count = 0
...
new_results_df = prepare_display_df(pd.DataFrame(new_rows)) if new_rows else pd.DataFrame()

col1, col2, col3, col4 = st.columns(4)
col1.metric("Total results", len(results_df))
col2.metric("New this run", len(new_rows))
...
if not new_results_df.empty:
    ...
    st.info("No new results found this run.")
